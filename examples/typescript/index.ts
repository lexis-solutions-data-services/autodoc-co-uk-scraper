...
async function runActor() {
  ...
  const actorId = `lexis-solutions/autodoc-co-uk-scraper`;
  console.log(`Starting actor: ${actorId}`);

  const { defaultDatasetId } = await client.actor(actorId).call({
    startUrls: [
      "https://www.autodoc.co.uk/car-parts/engine-oil-12094/bmw/1er-reihe/1-convertible-e88/25443-118-i",
      "https://www.autodoc.co.uk/mapco/2039778"
    ],
    maxItems: 10,
    proxyConfiguration: {
      "useApifyProxy": false
